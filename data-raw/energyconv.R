...
prep.energyconv <- function()
{
    ...
    fromEJ <-   c(1         , 1e6     , 1e12    , 1/3.6e-9 , 2.390e2 ) #1e18 J
    fromTJ <-   c(1e-6      , 1       , 1e6     , 1/3.6e-3 , 2.390e-04 ) #1e12 J
    fromMJ <-   c(1e-12     , 1e-6    , 1       , 1/3.6e3  , 2.390e-10 ) #1e6 J
    fromMWh <-  c(3.6e-9    , 3.6e-3  , 3.6e3   , 1        , 8.606e-07 )
    fromPcal <- c(4.184e-3  , 4.184e3 , 4.184e9 , 1.162e6  , 1 )
    cm <- matrix(c(fromEJ, fromTJ, fromMJ, fromMWh, fromPcal), nrow=5, byrow=TRUE)
    colnames(cm) <- c('EJ', 'TJ', 'MJ', 'MWh', 'Pcal')
