...
test_that('GETQ Mode returns correct query titles', {
    expect_match(module.population(GETQ), 'Population')
    expect_match(module.pcgdp_ppp_(GETQ), 'pcGDP\\(PPP\\)')
    expect_match(module.gdp_mer_(GETQ), "GDP\\(MER\\)")
...
test_that('Socioeconomics models produce data', {
              ...
              expect_identical(module.population(RUN, queries, aggkeys,
                                                 aggfn, years, filters,
                                                 'thous'),
                               dplyr::filter(queries$Population, year>=2000, year<=2050))
              expect_identical(module.gdp_mer_(RUN, queries , aggkeys,
                                               aggfn, years, filters, 'Million1990US$'),
                               dplyr::filter(queries$`GDP(MER)`, year>=2000, year<=2050))
              expect_identical(module.pcgdp_ppp_(RUN, queries ,
              ...
              ## module.
              popmil <- module.population(RUN, queries , aggkeys,
